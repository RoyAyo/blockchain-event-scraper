...
export const loadFeeCollectorHandler = async (job: Job, done: DoneCallback) => {
  const { maxBlockNo, fromBlock, toBlock } = job.data;
  ...
  try {
    ...
    console.error(
      `Error processing block ${fromBlock} -> ${toBlock} on attempt ${job.attemptsMade}`,
    );
    if (job.attemptsMade >= QUEUE_FAILED_RETRY - 1) {
      // to know what block range to retry
      redisClient.push("failed", `${fromBlock}-${toBlock}`);
      console.error(
        `Job Failed due to ${error} => Retry from: ${fromBlock}: to: ${toBlock}`,
      );
      job.remove();
    }
    done(error as Error);
...
loadFeeCollectorQueue.on("finished", async () => {
  ...
  try {
    ...
    for (const record of records) {
      ...
      const events: ParsedFeeCollectedEvents[] = JSON.parse(
        convertedRecord.events,
      );
